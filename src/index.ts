...
type FlagAge = keyof typeof flag_age_map

config()

async function run_action(): Promise<void> {
    const optimizely_project_id = getInput('optimizely_project_id') || process.env.OPTIMIZELY_PROJECT_ID || ""
    const optimizely_auth_token = getInput('optimizely_auth_token') || process.env.OPTIMIZELY_AUTH_TOKEN || ""

    /* Get all the flags */
    const flags: Array<any> = await getAllFlags({optimizely_project_id, optimizely_auth_token})
    ...
    active_flags.forEach((flag) => {
        ...
        if(flag_map.has(age_key)) {
            flag_map.get(age_key)?.push(flag)
        } else {
            flag_map.set(age_key, [flag])
        }
    })

    console.log(flag_map.forEach((flags, key) => {console.log(`${key}: ${flags.length} flags`)}))
}
